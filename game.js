...
function preload() { // 첫번째로 실행됨 -  주로 데이터 로딩
        ...
        this.load.audio('end_music_w', './7. 효과음/승리음.mp3');
        this.load.audio('end_music_f', './7. 효과음/패배음.mp3');
    ...
    this.load.image('menu_box_ui', './3. 메뉴창/메뉴창.png');
...
function create() {
    ...
    end_f = this.sound.add('end_music_f');
...
function sound_set(num){
    ...
    end_f.setVolume((start_setting_togle1.x - togle_min_x)/s);
...
async function button_fun(button1){//클릭 애니메이션 이후 버튼 발동
  ...
  {
    touch_m.play();
    copyTextToClipboard('떨어지는 센세 피하기 :' +game_score.toString()+'pt\n yuuka0314.github.io ');
...
function key_pc(){
        ...
        if(arrow.down.isDown && skill_cool == false){
            if(aris_power == 0 && character_choice_num == 3){
                play_character.skill_gaze -= 33;
                if(play_character.skill_gaze <= 0){
                    play_character.skill_gaze = 0;
                }
            ...
            if(character_choice_num == 3 && play_character.skill_gaze > 0){
...
function update() { // update 함수
    ...
    if(game_type =="in_game"){
        ...
        else if(ingame_type == "warning" && (Date.now() - timer)/1000 < 2) 
        {
            ...
            die_timer = 100;
            if(game_tier_score[3] < game_score){
                end_w.play();
            }
            else{
                end_f.play();
